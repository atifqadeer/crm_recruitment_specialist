...
{{--        <div class="page-header page-header-dark has-cover" style="border: 1px solid #ddd; border-bottom: 0;">--}}
        <div class="page-header page-header-dark has-cover">
            <div class="page-header-content header-elements-inline">
                <div class="page-title">
                    <h5>
                        <a href="{{ route('offices.index') }}"><i class="icon-arrow-left52 mr-2" style="color: white;"></i></a>
                        <span class="font-weight-semibold">Head Office</span> - Add
                    </h5>
                </div>
            </div>
            ...
            <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
                <div class="d-flex">
                    <div class="breadcrumb">
                        ...
                        <a href="#" class="breadcrumb-item">Head Office </a>
        ...
        <div class="content">
            <!-- Centered forms -->
            @if(session()->has('office_add_error'))
                <div class="alert alert-danger">
                    {{ session()->get('office_add_error') }}
                </div>
            @endif
                            ...
                            <div class="modal-dialog">
                                ...
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title">Write a Note for Office</h4>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea id="duplicate_note_for_office_id" cols="30"
                                                      rows="5" class="form-control"
                                                      placeholder="WRITE A NOTE FOR DUPLICATE OFFICES HERE..."></textarea>
                                        </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-10 offset-md-1">
                                    ...
                                    <div class="form-group">
                                        {{ Form::label('head_office_text','Head Office Name', array('class'=>'col-form-label')) }}
                                        {{ Form::text('head_office_name',null, array('id'=>'head_office_id','class'=>'form-control', 'placeholder' => 'ENTER HEAD OFFICE NAME')) }}
                                    </div>

                                    <div class="form-group">
                                        {{ Form::label('postcode','Postcode') }}
                                        {{ Form::text('office_postcode',null,array('id'=>'postcode_id','class'=>'form-control','placeholder' => 'ENTER OFFICE POSTCODE')) }}
                                    </div>
                                    <div class="form-group">
                                        {{ Form::label('email_address','Choose Type',array('class'=>'col-form-label')) }}
                                        {{ Form::select('office_type',array(
                                        ''=>'SELECT TYPE',
                                        'psl'=> 'PSL',
                                        'non psl'=> 'NON PSL'
                                        ),null,array('class' => 'form-control form-control-select2')) }}
                                    </div>
                                    <div class="container px-0">
                                        <div class="card p-4 d-none" id="contact-card-template">
                                            ...
                                            <div class="form-group">
                                                ...
                                                {{ Form::text('office_contact_name[]',null,array('id'=>'name_id','class'=>'form-control','placeholder' => 'ENTER NAME')) }}
                                            </div>
                                            <div class="form-group">
                                                {{ Form::label('phnumber','Phone Number') }}
                                                {{ Form::text('office_contact_phone[]',null,array('id'=>'phone_number_id','class'=>'form-control','placeholder' =>'ENTER PHONE NUMBER')) }}
                                            </div>
                                            <div class="form-group">
                                                {{ Form::label('mobile','Landline Number') }}
                                                {{ Form::text('office_landline[]',null,array('id'=>'home_number_id','class'=>'form-control','placeholder' => 'ENTER OFFICE LANDLINE NUMBER')) }}
                                            </div>
                                            <div class="form-group">
                                                {{ Form::label('email','Email Address') }}
                                                {{ Form::email('office_contact_email[]',null,array('id'=>'email_address_id','class'=>'form-control','placeholder' => 'ENTER EMAIL ADDRESS')) }}
                                            </div>
                                            <div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        {{ Form::text('office_contact_website',null,array('id'=>'website_id','class'=>'form-control','placeholder' =>'ENTER WEBSITE ADDRESS')) }}
                                    </div>
                                    <div class="form-group">
                                        {{ Form::label('office_notes', 'Add Notes:') }}
                                        {{ Form::textarea('office_notes',null,
...
<script>
    ...
    function getFormData() {
        const names = Array.from(document.querySelectorAll('input[name="office_contact_name[]"]'))
            .map(input => input.value.trim()).filter(value => value !== "");
        const phones = Array.from(document.querySelectorAll('input[name="office_contact_phone[]"]'))
            .map(input => input.value.trim()).filter(value => value !== "");
        const landlines = Array.from(document.querySelectorAll('input[name="office_landline[]"]'))
            .map(input => input.value.trim()).filter(value => value !== "");
        const emails = Array.from(document.querySelectorAll('input[name="office_contact_email[]"]'))
            .map(input => input.value.trim()).filter(value => value !== "");

        return {
            office_contact_name: names,
            office_contact_phone: phones,
            office_landline: landlines,
            office_contact_email: emails
        };
    }
