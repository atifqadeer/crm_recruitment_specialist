        ...
        if (targetTab === "#all_nurse_resources") {
            ...
            if (!allApplicants) {
                allApplicants = $('#all_nurse_resources_table').DataTable({
                    "processing": true,
                    "serverSide": true,
                    "ajax": "{!! url('getlast2MonthsApplicationAjaxTemp') !!}/" + job,
                    "order": [[0, 'desc']],
                    "columns": columns
                });
            } else {
            ...
            if (!notInterested) {
                notInterested = $('#not_interested_nurse_resources_table').DataTable({
                    "processing": true,
                    "serverSide": true,
                   "ajax": "{!! url('getlast2MonthsNotInterestedApplicationAjaxTemp') !!}/" + job,
                    "order": [[0, 'desc']],
                    "columns": columns
                });
            } else {
            ...
            if (!blockedApplicants) {
                blockedApplicants = $('#blocked_nurse_resources_table').DataTable({
                    "processing": true,
                    "serverSide": true,
                   "ajax": "{!! url('getlast2MonthsBlockedApplicationAjaxTemp') !!}/" + job,
                    "order": [[0, 'desc']],
